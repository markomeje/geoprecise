<div class="dashboard">
  @include('client.includes.header')
      ...
      <div class="container-fluid py-4">
        ...
        @else
          <?php $plan = $sib->plan; $model_id = $sib->id; $model = 'sib'; $layout = $plan->layout; $plot_numbers = $plan->plot_numbers; $approved = true === (boolean)($sib->approved ?? false); $completed = true === (boolean)($sib->completed ?? 0); ?>
          <div class="row">
            <div class="col-12 col-lg-7 col-xl-6">
                <div class="alert alert-dark mb-4 text-white border-0 d-flex justify-content-between align-items-center">
                    <span class="">Site Inspection</span>
                    @if($approved)
                ...
                @if(empty($plot_numbers))
                    <div class="alert alert-danger border-0 text-white mb-4" role="alert">Unknown error. Please contact our office</div>
                @else
                <?php $plot_numbers = str_contains($plot_numbers, '-') ? explode('-', $plot_numbers) : $plot_numbers; ?>
                <div class="card mb-4">
                    <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                    <div class="text-dark">Plot Numbers Lifted</div>
                    </div>
                    <div class="card-body">
                ...
                <?php $payment = $sib->payment; $amount = $sib->form ? ($sib->form->amount ?? 0) : 0; $total_plots = (is_array($plot_numbers) ? count($plot_numbers) : 1); $total_amount = $total_plots * (int)$amount; ?>
                @if(empty($payment))
                ...
                @else
                    <div class="p-4 bg-white mb-4 border">
                        ...
                        @else
                            <div class="alert alert-info mb-0 text-white">Payment of <span class="font-weight-bolder">NGN{{ number_format($payment->amount) }}</span> Awaiting Approval</div>
                        @endif
                    </div>
                @endif
              @endif
                    ...
                    <form class="save-sib-form" action="javascript:;" method="post" data-action="{{ route('client.sib.save', ['id' => $sib->id]) }}">
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <input id="" class="form-control phone" type="text" name="phone" placeholder="Enter phone" value="{{ $sib->phone }}">
                                <small class="phone-error text-danger"></small>
                            </div>
                            <div class="form-group col-md-6">
                                <label for="">Address</label>
                                <input id="" class="form-control address" type="text" name="address" placeholder="Enter address" value="{{ $sib->address }}">
            ...
            <div class="col-12 col-lg-5 col-xl-6">
                <div class="alert alert-dark text-white mb-4 border-0">Other Booking</div>
                <?php $sibs = \App\Models\Sib::where('id', '!=', $model_id)->where(['client_id' => $sib->client_id])->take(6)->get(); ?>
                @if(empty($sibs->count()))
                  <div class="alert alert-danger text-white mb-4 border-0">No Other Booking available</div>
                @else
                  <div class="row">
                    @foreach($sibs as $sib)
