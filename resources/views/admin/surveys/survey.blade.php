<div class="dashboard">
  @include('admin.includes.header')
      ...
      <div class="container-fluid py-4">
        ...
        @else
          <?php $client_id = $survey->client->id; $model_id = $survey->id; $model = 'survey'; $layout = $survey->layout; $form = $survey->form; $plot_numbers = $survey->plot_numbers; $client_name = $survey->client->fullname; $approved = (true === (boolean)$survey->approved); $payment = $survey->payment; $paid = empty($payment) ? false : ($payment->status === 'paid' ? true : false); ?>
          <div class="min-vh-100">
            <div class="row">
              <div class="col-12 col-lg-8 mb-4">
                ...
                @else
                  <div class="card border mb-4">
                    <div class="card-header border-bottom">
                      @can('create', ['plot-numbers'])
                        <div class="row">
                          <div class="col-12 mb-4">
                            <div class="cursor-pointer w-100 text-center text-white d-block bg-primary border border-primary p-4" data-bs-toggle="modal" data-bs-target="#add-client-plot">Add Plot Numbers.</div>
                          </div>
                        </div>
                    ...
                    <div class="card-body pb-2">
                                ...
                                @foreach($plot_numbers as $number)
                                    @if(!empty($number))
                                      <div class="col-12 col-md-6 col-lg-4 mb-4">
                                          <div class="bg-dark rounded-0 border d-flex align-items-center justify-content-between p-3">
                                            <span class="text-white">
                                              {{ $number }}
                                            </span>
                                            @can('delete', ['plot-numbers'])
                                              <small class="text-danger tiny-font cursor-pointer client-delete-plot" data-url="{{ route('admin.plot.number.delete', ['plot_number' => $number, 'model_id' => $model_id, 'model' => $model]) }}" data-message="Are you sure to delete?">
                        ...
                        @else
                            <div class="bg-dark rounded-0 p-3 mb-3 d-flex align-items-center justify-content-between">
                                <span class="text-white">{{ $plot_numbers }}</span>
                                @can('delete', ['plot-numbers'])
                                  <small class="text-danger tiny-font cursor-pointer client-delete-plot" data-url="{{ route('admin.plot.number.delete', ['plot_number' => $plot_numbers, 'model_id' => $model_id, 'model' => $model]) }}" data-message="Are you sure to delete?">
                                    <i class="icofont-trash"></i>
                                  </small>
                                @endcan
                            </div>
                ...
                @endif
                @if(empty($payment))
                  <div class="alert alert-danger border-0 mb-4 text-white">No Payment made for this application.</div>
                @else
                ...
                <div class="">
                  <form class="edit-survey-form" action="javascript:;" data-action="{{ route('admin.survey.edit', ['id' => $model_id]) }}" method="post">
                    <div class="card border mb-4">
                    ...
                    <div class="alert d-none edit-survey-message mb-4 text-white"></div>
                    @can('update', ['surveys'])
                      <button class="btn btn-primary btn-lg w-100 edit-survey-button mb-4">
                        <img src="/images/spinner.svg" class="me-2 d-none edit-survey-spinner mb-1">Save
                      </button>
                    @else
                      <div class="alert alert-danger text-white" role="alert">
                          You do not have the permission to edit surveys
                      </div>
                    @endcan
                    @if($approved)
                      ...
                      @if($paid)
                        @can('approve', ['surveys'])
                          <div class="approve-survey" data-url="{{ route('admin.survey.approve', ['id' => $model_id]) }}">
                            <button class="btn btn-info btn-lg w-100 approve-survey-button mb-0">
                              <img src="/images/spinner.svg" class="me-2 d-none approve-survey-spinner mb-1">Approve
                            </button>
                          </div>
                        @else
                            <div class="alert alert-danger text-white" role="alert">
